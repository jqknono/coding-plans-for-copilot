...
const PRICING_PROBE_CONCURRENCY = Math.max(
  1,
  Number.parseInt(process.env.OPENROUTER_PRICING_PROBE_CONCURRENCY || '4', 10),
);
...
function extractTierPlansFromScript(jsText) {
  ...
  for (const match of matched) {
    ...
    const rawPrice = String(match[3] || '')
      .trim()
      .toLowerCase();
    const isTierName =
      /^(free|basic|starter|base|plus|pro|team|business|premium|enterprise)$/.test(id) ||
...
function extractEnrichedTierPlans(jsText) {
  ...
  while ((match = planRegex.exec(text)) !== null) {
    ...
    if (featuresMatch) {
      features = [...featuresMatch[1].matchAll(/"([^"]{3,200})"/g)].map((m) => m[1].trim()).filter(Boolean);
    }
...
function extractServiceDetailCandidates(htmlText, limit = 6) {
  ...
  const focusHtml = anchorIndex >= 0 ? source.slice(Math.max(0, anchorIndex - 25_000), anchorIndex + 35_000) : source;
  ...
  const shouldKeep = (value) => {
    ...
    const detailSignal =
      /[\u4e00-\u9fff]|\d|support|model|models|prompt|request|quota|limit|storage|api|private|unlimited|access|credit|image|text|code|encrypt|upscal|tool|兼容|支持|额度|请求|模型|每月|每周|每天|小时/i;
  ...
  for (const match of rowMatches) {
    const cells = [...String(match[1] || '').matchAll(/<t[hd][^>]*>([\s\S]*?)<\/t[hd]>/gi)].map((item) =>
      stripHtmlTags(item[1]),
    );
    if (cells.length < 2) {
      continue;
    }
...
function filterSubscriptionPriceTokens(tokens) {
  return (tokens || []).filter((token) => {
    ...
    return amount !== null && amount >= HEURISTIC_MIN_SUBSCRIPTION_PRICE && amount <= HEURISTIC_MAX_SUBSCRIPTION_PRICE;
  });
}
...
function hasSubscriptionSignal(htmlText) {
  const text = compactText(htmlText).toLowerCase();
  return /(subscription|per month|monthly|per year|yearly|annual|annually|month-to-month|membership|套餐|包月|月付|年付|会员|\/\s*mo\b)/i.test(
    text,
  );
}
...
async function probePricingFromRuntimeAssets(candidateUrl, htmlText, officialWebsiteUrl) {
  ...
  const entryUrls = extractRuntimeJsEntryUrls(htmlText, candidateUrl).filter((url) =>
    isSameOrSubdomain(url, officialWebsiteUrl || candidateUrl),
  );
  ...
  while (queue.length > 0 && visited.size < PRICING_BUNDLE_PROBE_MAX_FILES) {
    ...
    const validTierPlans = tierPlans.filter(
      (item) =>
        Number.isFinite(item.price) &&
        item.price >= HEURISTIC_MIN_SUBSCRIPTION_PRICE &&
        item.price <= HEURISTIC_MAX_SUBSCRIPTION_PRICE,
...
async function parseMistralCustomPricing() {
  ...
  const extractTableFeatures = (columnName) => {
    ...
    for (const tr of html.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi)) {
      const cells = [...tr[1].matchAll(/<t[hd][^>]*>([\s\S]*?)<\/t[hd]>/gi)].map((c) => stripHtmlTags(c[1]).trim());
      if (cells.length >= 3) {
        rows.push(cells);
      }
    }
...
function extractCloudflareLlmExamples(markdownText, limit = 6) {
  const section = extractTextSection(
    markdownText,
    /##\s+LLM model pricing/i,
    /##\s+(?:Embeddings|Image|Audio|Other) model pricing/i,
  );
  const rows = [];
...
function extractRscPlanFeatures(rscPayload, planTitle) {
  ...
  const planPattern = new RegExp(`"_title":"${titleEsc}"[^}]*"priceMonthly":"([^"]+)"`);
...
function extractChutesTierInfoFromText(plainText) {
  ...
  const frontierRequirement = text.match(/Frontier models[^.]{0,260}require a Plus plan or higher\.?/i)?.[0] || null;
  const tierMap = new Map();

  for (let i = 0; i < tierNames.length; i += 1) {
    const tier = tierNames[i];
    const nextTier = tierNames[i + 1] || 'Enterprise';
    const blockRegex = new RegExp(`\\b${tier}\\b([\\s\\S]{0,900}?)(?=\\b${nextTier}\\b|$)`, 'i');
    ...
    const nearTierPriceMatch = text.match(new RegExp(`\\b${tier}\\b[\\s\\S]{0,80}?\\$\\s*([0-9]+(?:\\.[0-9]+)?)`, 'i'));
...
async function parseChutesCustomPricing() {
  ...
  const entryUrls = extractRuntimeJsEntryUrls(html, sourceUrl).filter((url) =>
    isSameOrSubdomain(url, 'https://chutes.ai'),
  );
    ...
    try {
      ...
      for (const nodeUrl of unique(nodeUrls)) {
        try {
          ...
          const chunkUrls = extractModuleJsUrls(nodeText, nodeUrl).filter((url) =>
            isSameOrSubdomain(url, 'https://chutes.ai'),
          );
...
async function parseRedpillCustomPricing() {
  ...
  const displayedPriceToken =
    ...
    text.match(
      /Pro\s+For individuals(?:\s*&\s*|\s+and\s+)professionals[\s\S]{0,600}?(\$\s*[0-9]+(?:\.[0-9]+)?\s*\/\s*month)/i,
    )?.[1] ||
    null;
  ...
  const inferredMonthlyPrice =
    Number.isFinite(annualSavePercent) && annualSavePercent > 0 && annualSavePercent < 100
  ...
  if (monthlyPrice > displayedPrice && Number.isFinite(annualSavePercent)) {
    notes.push(
      `页面展示年付折算价 $${displayedPrice}/month（Save ${annualSavePercent}%），换算月付价 $${monthlyPrice}/month`,
    );
  }
...
async function parseZAiCustomPricing() {
  ...
  try {
    ...
    await page.evaluate(() => {
      const normalize = (value) =>
        String(value || '')
          .replace(/\s+/g, ' ')
          .trim();
      const monthlyTab = Array.from(document.querySelectorAll('*')).find(
        (node) => normalize(node.textContent) === 'Monthly',
      );
      if (monthlyTab) {
        monthlyTab.click();
      }
    ...
    const extractedPlans = await page.evaluate(() => {
      ...
      const seen = new Set();
      const cards = [];
      const subscribeButtons = Array.from(document.querySelectorAll('button')).filter((node) =>
        /Subscribe/i.test(normalize(node.textContent)),
      );
      ...
      for (const button of subscribeButtons) {
        ...
        const renewalText =
          texts.find((text) => /^\$\s*[0-9]+(?:\.[0-9]+)?\s*\/\s*month from 2nd month$/i.test(text)) || null;
        const featureTitle =
          texts
...
async function probeOfficialPricing(openrouterProvider, officialWebsiteUrl) {
  ...
  const linksFromHomepage = extractPricingLinks(homepageText, homepageUrl).filter((link) =>
    isSameOrSubdomain(link, officialWebsiteUrl),
  );
  const providerSlug = String(openrouterProvider?.slug || '')
    .trim()
    .toLowerCase();
  const preferredPricingUrl =
    providerSlug && PRICING_PAGE_OVERRIDES[providerSlug] ? PRICING_PAGE_OVERRIDES[providerSlug] : null;
  const pathCandidates = PRICING_PATH_CANDIDATES.map((candidatePath) => absoluteUrl(candidatePath, homepageUrl)).filter(
    Boolean,
  );

  const candidateUrls = unique([preferredPricingUrl, ...pathCandidates, ...linksFromHomepage, homepageUrl]).slice(
    0,
    PRICING_PROBE_MAX_CANDIDATES,
  );
  ...
  for (const candidateUrl of candidateUrls) {
    try {
      ...
      if (
        ...
        planLike &&
        validRecurringPriceLike &&
        (candidateLooksPricing || subscriptionLike || tierLike)
      ) {
        return {
      ...
      if (!runtimeProbeTried && candidateLooksPricing && planLike) {
        ...
        const runtimeProbe = await probePricingFromRuntimeAssets(
          page.url || candidateUrl,
          page.text,
          officialWebsiteUrl,
        );
  ...
  if (usageOnlyEvidence) {
    const reason =
      usageOnlyEvidence.subscriptionLike && usageOnlyEvidence.recurringLike
  ...
  const reason = probeErrors.length > 0 ? `官网价格页探测失败：${probeErrors[0]}` : '官网未发现可解析的套餐价格页';
...
async function main() {
  ...
  const probed = await mapWithConcurrency(providersNeedingProbe, PRICING_PROBE_CONCURRENCY, async (item) => {
    ...
    const slug = String(openrouterProvider?.slug || '')
      .trim()
      .toLowerCase();
    ...
    const fallbackReason = normalizedPlans.length > 0 ? '检测到套餐价格，但未提取到有效服务详情' : probeResult.reason;
  ...
  const output = {
    ...
    summary: {
      ...
      structuredProviderCount: mappedProviders.filter((item) => item.parseMode === 'provider-pricing-structured')
        .length,
