...
function parseTierPriceBreakdown(rawValue) {
  ...
  const renewalAmount = readAmount(/(?:续费|恢复)[^0-9]{0,16}([0-9]+(?:,[0-9]{3})*(?:\.[0-9]+)?)(?:\s*元)?\s*\/\s*月/i);
  const monthlyAmount =
    thirdMonthAmount ?? renewalAmount ?? readAmount(/([0-9]+(?:,[0-9]{3})*(?:\.[0-9]+)?)\s*元?\s*\/\s*月/i);
...
function parseJdCloudCodingPlansFromText(pageText) {
  ...
  for (const match of planMatches) {
    ...
    const detailMatches = detailText.matchAll(
      /(模型|工具|权益|月用量)：\s*([^：]+?)(?=(?:模型|工具|权益|月用量)：|$)/g,
    );
    const details = [];
...
function parseKimiFeatureCandidates(bundleText) {
  ...
  while ((planMatch = planRegex.exec(bundleText)) !== null) {
    ...
    const features = unique(
      [...featureBlob.matchAll(/text:"([^"]+)"/g)].map((item) => stripSimpleMarkdown(item[1])).filter(Boolean),
    );
...
function pickKimiFeaturesByTitleAndPrice(candidates, title, currentPrice) {
  ...
  const exact = matches.find(
    (item) => Number.isFinite(item.price) && Number.isFinite(currentPrice) && item.price === currentPrice,
  );
  if (exact) {
...
function parseJdCloudCodingPlansFromDocsText(pageText) {
  ...
  const sharedQuotaText =
    /多工具之间套餐额度共享/.test(text) && !/多工具之间套餐额度共享/.test(toolsText) ? '多工具之间套餐额度共享' : null;
...
function mergeJdCloudPricingAndDocsPlans(pricingPlans, docsPlans) {
  ...
  return dedupePlans(
    (pricingPlans || []).map((plan) => {
      const tier = normalizeText(plan?.name || '')
        .replace(/^Coding\s+Plan\s+/i, '')
        .toLowerCase();
  ...
  try {
    ...
    const buyHref = await page.evaluate(() => {
      const link = Array.from(document.querySelectorAll('a')).find((anchor) =>
        /购买页/.test(String(anchor.textContent || '')),
      );
...
async function parseZhipuCodingPlansWithPlaywright() {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      const monthlyTab =
        Array.from(document.querySelectorAll('.switch-tab-item')).find((node) =>
          /连续包月/.test(normalize(node.textContent)),
    ...
    if (!/连续包月/.test(extracted?.activeTabText || '')) {
      throw new Error(
        `Unable to switch Zhipu pricing page to monthly tab (active: ${extracted?.activeTabText || 'unknown'})`,
      );
    }
  ...
  const sortedCards = [...selectedCards]
    .filter(
      (item) =>
        ...
        String(item.unit).toLowerCase() === 'month',
    )
    .sort((left, right) => {
...
async function parseZhipuCodingPlansFromClaudeCodeBundle() {
  ...
  const claudeCodeChunkHash =
    appJs.match(/(?:^|[,\{])ClaudeCode:"([0-9a-f]+)"/i)?.[1] || appJs.match(/"ClaudeCode":"([0-9a-f]+)"/i)?.[1];
  ...
  const plans = dedupePlans(
    cardsToBuild
      ...
      .map((card) => {
        const monthlyAmount =
          Number.isFinite(card.monthStep) && card.monthStep > 0 ? card.salePrice / card.monthStep : card.salePrice;
        ...
        const notes = [
          card.tagText || '',
          Number.isFinite(card.renewAmount)
            ? `${renewLabelByUnit[card.unit] || '续费金额'}：¥${formatAmount(card.renewAmount)}`
            : '',
        ]
          .filter(Boolean)
        ...
        const normalizedServiceDetails = normalizeServiceDetails(
          serviceDetails || [card.attrTitle || ''].filter(Boolean),
        );
...
async function parseMinimaxCodingPlans() {
  ...
  const apiUrl =
    'https://www.minimaxi.com/public/api/openplatform/charge/combo/products?cycle_type=1&biz_line=2&resource_package_type=7';
  ...
  const plans = dedupePlans(
    packages
      ...
      .map((item) => {
        ...
        return asPlan({
          name: item.title,
          currentPriceText: Number.isFinite(currentPrice)
            ? `¥${formatAmount(currentPrice)}/月`
          ...
          notes: usageText
            ? `用量: ${usageText}`
            : Number.isFinite(renewPrice)
              ? `续费金额：¥${formatAmount(renewPrice)}/月`
              : null,
...
async function parseBaiduCodingPlans() {
  ...
  const usageRow = serviceRows.find((row) => /每月限额|月用量|用量限制/i.test(normalizeText(row?.[0] || '')));
  const modelRow = serviceRows.find((row) => /支持模型/i.test(normalizeText(row?.[0] || '')));
...
async function parseTencentCodingPlans() {
  ...
  if (planHeaderIndex < 0) {
    await ensureFallbackData();
    planHeaderIndex = rows.findIndex((row) => /lite\s*套餐/i.test(row.join(' ')) && /pro\s*套餐/i.test(row.join(' ')));
  }
  if (planHeaderIndex < 0) {
  ...
  const pricingRowLabels = new Set([
    '价格',
    '套餐价格',
    '刊例价',
    '原价',
  ...
  let priceRow = pricingTableRows.find((row) =>
    ['价格', '套餐价格', '刊例价', '原价'].includes(normalizeText(row?.[0] || '')),
  );
  if (!priceRow) {
    await ensureFallbackData();
    planHeaderIndex = rows.findIndex((row) => /lite\s*套餐/i.test(row.join(' ')) && /pro\s*套餐/i.test(row.join(' ')));
    ...
    priceRow = pricingTableRows.find((row) =>
      ['价格', '套餐价格', '刊例价', '原价'].includes(normalizeText(row?.[0] || '')),
    );
  }
  if (!priceRow) {
  ...
  const promoPriceRow = pricingTableRows.find((row) =>
    /(?:限时)?特惠价格|优惠价格|活动价格/i.test(normalizeText(row?.[0] || '')),
  );
...
async function parseJdCloudCodingPlans() {
  ...
  const plans = pricingPlans.length > 0 ? mergeJdCloudPricingAndDocsPlans(pricingPlans, docsPlans) : docsPlans;
...
async function fetchXAioTextWithRetry(url, options = {}, attempts = 2) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      const isRetryable =
        /(?:\b5\d{2}\b)|fetch failed|timeout|timed out|ECONNRESET|ETIMEDOUT|EAI_AGAIN|socket hang up/i.test(message);
...
function parseCompshareCodingPlansFromHtml(html) {
  ...
  const priceRow =
    rows.find((row) => {
      const label = normalizeText(row?.[0] || '');
      return (
        /^(月费|价格|套餐价格)$/.test(label) && (row || []).some((cell, index) => index > 0 && isMonthlyPriceText(cell))
      );
    }) || null;
  ...
  const featureMatches = [
    {
      ...
      descPattern: /支持千问系列模型[\s\S]*?Qwen3\.5-Plus[\s\S]*?更多模型持续接入中。/,
    },
    {
      label: '工具',
...
function parseAliyunServiceDetailsFromDocsHtml(html) {
  ...
  const proColumn =
    rows
      .find((row) => row.some((cell) => /Pro\s*高级套餐/i.test(cleanupDocsValue(cell || ''))))
      ?.findIndex((cell) => /Pro\s*高级套餐/i.test(cleanupDocsValue(cell || ''))) ?? -1;
...
async function parseVolcengineCodingPlans() {
  ...
  for (const candidate of unique([...candidates.slice(0, 2), fallbackIndexUrl])) {
    ...
    selectedPlans = plans;
    if (
      plans.every((plan) => plan.currentPriceText && plan.originalPriceText && (plan.serviceDetails || []).length >= 3)
    ) {
      break;
    }
  }
...
async function parseCucloudCodingPlans() {
  ...
  const plans = [
    asPlan({
      ...
      unit: '月',
      notes: '活动限量免费名额，先到先得',
      serviceDetails: ['特供 Lite 套餐', '1.8 万次/月', '与 AI 云桌面、AI 云主机、AI 云容器活动联动'],
    }),
    asPlan({
      ...
      unit: '月',
      notes: '活动限量免费名额，先到先得',
      serviceDetails: ['特供 Pro 套餐', '最高 9 万次/月', 'Lite 与 Pro 共计 12000 个专属免费名额'],
    }),
  ];
