...
当用户已在 `Manage Language Models` 中显式添加 `Coding Plans` provider group 时，`Coding Plans` 在扩展激活后异步完成模型初始化，Copilot Chat 的 `Pick Model` 应能看到这些模型，而不要求用户先手动执行 `Coding Plans: Refresh Models`。

说明：未显式添加 group 的 `coding-plans` 根 provider 默认保持隐藏，不再直接暴露模型。

## Scenarios

| Scenario | Given | When | Then |
| --- | --- | --- | --- |
| 首次激活后无需手动刷新即可枚举模型 | 用户已显式添加一个 `Coding Plans` provider group，且其对应的 `coding-plans.vendors` 预先包含至少一个静态模型，扩展尚未激活 | 激活扩展并等待初始化完成 | 该显式 provider group 下可看到对应模型，无需先手动执行刷新命令。 |
